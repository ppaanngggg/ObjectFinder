...
def hog(image,cell_size,block_size,row_num,col_num,bar):
    ...
    norm_angle=np.uint16(norm_angle/(np.pi/bar))
    cell_table=np.zeros((row_num,col_num,bar))
    ...
    for row in range(row_num-block_size+1):
        for col in range(col_num-block_size+1):
            ...
            block_vec=block_vec/np.sqrt(np.sum(block_vec**2))
